...
public class CS400Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path copyPath, Edge extendBy) {
			...
			this.dataSequence = cloneHelper(copyPath.dataSequence, extendBy.target);
			this.end = extendBy.target;
		}

		/**
		 * This helper method creates a new Deep copy of the list that was previously given
		 * and adds the new target edge onto the list sequence without changing the previous 
		 * sequence.
		 * @param list
		 * @return newList a deep copy of the new dataSequence
		 */
		private LinkedList<NodeType> cloneHelper(List<NodeType> list, Vertex extendBy ) {
			LinkedList<NodeType> newList = new LinkedList<>();
			for(int i = 0; i < list.size(); i++) {
				newList.add(list.get(i));
			}
			newList.add(extendBy.data);
			return newList;  
		}
	...
	/*
	 * We have included an import statement for the java.util.PriorityQueue class. You should 
	 * create an instance of this class within the dijkstrasShortestPath method to hold the 
	 * discovered paths (including total cost distance, and previous vertex data) that have not 
	 * yet been determined to be the shortest possible paths to their given end vertex. Sometimes 
	 * the vertices within these paths are called the frontier.
	 */
	protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
		if(vertices.isEmpty()) {
			throw new NoSuchElementException("Failed to find path from start to end node.");
		}
		LinkedList<Path> vertex = new LinkedList<Path>();
		PriorityQueue<Path> queue = new PriorityQueue<Path>(); //shortest paths queue

		if(vertices.containsKey(start)) { //if vertices contains starting vertex, begin algorithm
			Path startPath = new Path(vertices.get(start));
			vertices.get(start).isVisited = true;
			queue.add(startPath);
			for(int i = 0; i < vertices.get(start).edgesLeaving.size(); i++) {
				queue.add(new Path(startPath, vertices.get(start).edgesLeaving.get(i))); //initial edges from starting node
				vertex.add(new Path(startPath, vertices.get(start).edgesLeaving.get(i)));
			}
		} 
		//go through all edges connected to starting vertex and find paths
		for(int i = 0; i < vertex.size(); i++ ) {
			if(!vertex.get(i).end.isVisited) {
				for(int j = 0; j < vertex.get(i).end.edgesLeaving.size(); j++) {
					queue.add(new Path(vertex.get(i), vertex.get(i).end.edgesLeaving.get(j))); //add paths to queue and linked list for boolean isVisited
					vertex.add(new Path(vertex.get(i), vertex.get(i).end.edgesLeaving.get(j)));
					vertex.get(i).end.isVisited = true; //mark vertex as visited after going through edges
				}
			}
		}		
		while(!queue.isEmpty()) { //go through the queue and check for match
			if(queue.peek().end.data.equals(end)) {
				return queue.poll();
			}
			else {
				queue.remove();
